...
├─────────────────────────────────────────────────────────────┤
│                    存储层                                    │
│  MySQL │ TDengine │ Qdrant │ Redis │ MinIO            │
├─────────────────────────────────────────────────────────────┤
│                    采集层                                    │
...
├── .env.example            # 环境变量模板
├── config/
│   ├── dev.yaml            # 开发环境配置（Docker Compose 使用）
│   ├── prod.yaml           # 生产环境配置
│   └── templates/          # 配置模板（含设备、模块模板）
...
1. `config/dev.yaml`（Docker Compose 默认使用）
2. `config/prod.yaml`（设置 `CONFIG_FILE=/app/config/prod.yaml` 时使用）
3. 环境变量 `.env`
...
当前网络环境下推荐从 ModelScope 下载：

```bash
# 创建目录
mkdir -p /tmp/model_cache_35

# ModelScope 下载（推荐，国内直连）
# 访问 https://www.modelscope.cn/models/qwen/Qwen3.5-9B DeepSeek-V4-Flash-GGUF
# 下载 qwen3.5-9b-deepseek-v4-flash-q8_0.gguf 到 /tmp/model_cache_35/
```
